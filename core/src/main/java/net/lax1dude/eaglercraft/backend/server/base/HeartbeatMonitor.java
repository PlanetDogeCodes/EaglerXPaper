...
public class HeartbeatMonitor {
    ...
    public void track(Channel channel) {
        if (channel != null && channel.isActive()) trackedConnections.put(channel, new AtomicInteger(0));
    }
    ...
    public void tick() {
        for (Map.Entry<Channel, AtomicInteger> entry : trackedConnections.entrySet()) {
            ...
            try { channel.writeAndFlush(new PingWebSocketFrame(Unpooled.EMPTY_BUFFER)); }
            catch (Throwable t) { trackedConnections.remove(channel); continue; }
            if (missed.incrementAndGet() > maxMissedPongs) {
